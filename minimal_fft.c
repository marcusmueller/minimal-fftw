...
int main(int argc, char *argv[]) {
  ...
  addr_in = mmap(NULL, length, PROT_READ, MAP_PRIVATE, fd_in, 0);
  ...
  int fallocate_return =
      posix_fallocate(fd_out, 0, sizeof(float) * num_ffts * fftlen);

  printf(
      "address mapping %p, length %d, num_ffts %d, posix_fallocate said %d \n",
  ...
  fftwf_plan my_plan;
  out =
      (fftwf_complex *)fftwf_malloc(sizeof(fftwf_complex) * fftlen * num_ffts);
  int n[] = {fftlen};
